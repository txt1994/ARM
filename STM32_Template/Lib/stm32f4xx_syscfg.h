/**
  ******************************************************************************
  * 文件:    stm32f4xx_syscfg.h
  * 作者:    MCD Application Team
  * 版本:    V1.8.0
  * 日期:    04-November-2016
  * 简介:    该文件包含 SYSCFG 固件库的所有功能原型。
  ******************************************************************************
  * @attention
  *
...
/** @defgroup BITSTREAM_CLOCK  位流时钟源选择
  * @{
  */
...
/** @defgroup MCHDLY_CLOCK  MCHDLY时钟启用
  * @{
  */
...
/** @defgroup DFSDM_CLOCKIN_SOURCE   DFSDM时钟输入源选择
  * @{
  */
...
/** @defgroup DFSDM_CLOCKOUT_SOURCE   DFSDM时钟源选择
  * @{
  */
...
/** @defgroup DFSDM_DATAIN0_SOURCE   DATAIN0的DFSDM源选择
  * @{
  */
...
/** @defgroup DFSDM_DATAIN2_SOURCE   DATAIN2的DFSDM源选择
  * @{
  */
...
/** @defgroup DFSDM_DATAIN4_SOURCE   DATAIN4的DFSDM源选择
  * @{
  */
...
/** @defgroup DFSDM_DATAIN6_SOURCE   DATAIN6的DFSDM源选择
  * @{
  */
...
/** @defgroup DFSDM_CLKIN_SOURCE   CLKIN的DFSDM1源选择
  * @{
  */
...
/** @defgroup DFSDM_CLKIN_SOURCE   CLKIN的DFSDM2源选择
  * @{
  */
...
void    SYSCFG_DeInit(void); // 将备用函数(重映射和 EXTI 配置)寄存器取消初始化为其默认重置值。
void    SYSCFG_MemoryRemapConfig(uint8_t SYSCFG_MemoryRemap); // 更改指定引脚的映射。
void    SYSCFG_MemorySwappingBank(FunctionalState NewState); // 启用或禁用内部闪存组交换。
void    SYSCFG_EXTILineConfig(uint8_t EXTI_PortSourceGPIOx, uint8_t EXTI_PinSourcex); // 选择用作 EXTI 线路的 GPIO 引脚。
void    SYSCFG_ETH_MediaInterfaceConfig(uint32_t SYSCFG_ETH_MediaInterface); // 选择以太网媒体接口。
void    SYSCFG_CompensationCellCmd(FunctionalState NewState); // 启用或禁用 I/O 补偿单元。
FlagStatus SYSCFG_GetCompensationCellStatus(void); // 检查是否设置了 I/O 补偿单元就绪标志。
#if defined(STM32F410xx) || defined(STM32F412xG) || defined(STM32F413_423xx)
void    SYSCFG_BreakConfig(uint32_t SYSCFG_Break); // 将所选参数连接到 TIM1 的中断输入。
#endif /* STM32F410xx || STM32F412xG || STM32F413_423xx */
#if defined(STM32F413_423xx)
